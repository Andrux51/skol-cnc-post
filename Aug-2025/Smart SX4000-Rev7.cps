...
properties = {
  ...
  smoothingRoughing: {
    title: "Smoothing Roughing",
    description: "Smoothing Roughing",
    group: "smoothing",
    type: "integer",
    value: 1,
    range: [0, 99999],
  ...
  smoothingSemiRoughing: {
    title: "Smoothing Semi-Roughing",
    description: "Smoothing Semi-Roughing",
    group: "smoothing",
    type: "integer",
    value: 4,
    range: [0, 99999],
  ...
  smoothingSemiFinishing: {
    title: "Smoothing Semi-Finishing",
    description: "Smoothing Semi-Finishing",
    group: "smoothing",
    type: "integer",
    value: 7,
    range: [0, 99999],
  ...
  smoothingFinishing: {
    title: "Smoothing Finishing",
    description: "Smoothing Finishing",
    group: "smoothing",
    type: "integer",
    value: 10,
    range: [0, 99999],
...
wcsDefinitions = {
  useZeroOffset: false,
  wcs: [{ name: "Extended", format: "G54.1 P", range: [1, 300] }],
};
...
var toolVectorOutputI = createOutputVariable(
  { prefix: "I", control: CONTROL_FORCE },
  ijkFormat
);
var toolVectorOutputJ = createOutputVariable(
  { prefix: "J", control: CONTROL_FORCE },
  ijkFormat
);
var toolVectorOutputK = createOutputVariable(
...
var gAbsIncModal = createOutputVariable(
  {
    onchange: function () {
      if (skipBlocks) {
...
var settings = {
  coolant: {
    ...
    coolants: [
      ...
      { id: COOLANT_MIST },
      { id: COOLANT_THROUGH_TOOL, on: 58, off: 59 },
      { id: COOLANT_AIR },
      { id: COOLANT_AIR_THROUGH_TOOL },
      { id: COOLANT_SUCTION },
      { id: COOLANT_FLOOD_MIST },
      { id: COOLANT_FLOOD_THROUGH_TOOL, on: [8, 58], off: [9, 59] },
...
function onOpen() {
  ...
  writeBlock(mFormat.format(61), formatComment("CHECK PALLET #1"));
  writeBlock(mFormat.format(62), formatComment("CHECK PALLET #2"));
  writeBlock(
  ...
  for (var i = 0; i < getNumberOfSections(); ++i) {
    ...
    var abc = getWorkPlaneMachineABC(section, false);
    addToArrayIfNotExists(vectors, abc.y);
  }
  for (var i = 0; i < vectors.length; ++i) {
    ...
    writeBlock(
      gFormat.format(65),
      "P9018",
      "B" + abcFormat.format(vectors[i]),
      "J1",
      "J" + j
    );
    dwoArray.push({ angle: vectors[i], wcs: j });
  }

  //writeln("length=" + dwoArray.length);
  /* dwoArray.forEach(function (element) {
    writeComment("ANGLE=" + element.angle);
  });*/
...
function addToArrayIfNotExists(arr, value) {
...
function setSmoothing(mode) {
  ...
  if (mode) {
    ...
    if (getProperty("useSmoothing") == "0") {
      writeBlock(gFormat.format(5.1), "Q1", formatComment("AICC ON"));
    } else {
      if (!useNanoSmoothing) {
        writeBlock(
          gFormat.format(5.1),
    ...
    // disable smoothing
    writeBlock(gFormat.format(5.1), "Q0", formatComment("AICC OFF"));
  }
...
function onSection() {
  ...
  var preloadToolOutput = [];
  if (insertToolCall) {
    preloadToolOutput = writePreloadTool();
  }

  var spindleSpeedOutput = [];
  if (
    !isTappingCycle() ||
    (isTappingCycle() && !(getProperty("useRigidTapping") == "without"))
  ) {
    spindleSpeedOutput = startSpindle(tool, insertToolCall);
  }
  ...
  if (insertToolCall || operationNeedsSafeStart || newWorkPlane) {
  ...
  wcsOutput = writeWCS(currentSection, wcsIsRequired);
  ...
  var coolantOutput = setCoolant(tool.coolant); // writes the required coolant codes
...
function onCommand(command) {
  switch (command) {
    case COMMAND_COOLANT_OFF:
      writeBlock(setCoolant(COOLANT_OFF));
      return;
...
function writeWCS(section, wcsIsRequired) {
  ...
  if (true) {
    ...
    writeStartBlocks(wcsIsRequired, function () {
      var abc = getWorkPlaneMachineABC(section, false);
      dwoArray.forEach(function (element) {
        if (element.angle == abc.y) {
          wcs = element.wcs;
        }
      });
      output.push([gFormat.format(54.1), "P" + wcs]);
    });
    currentWorkOffset = wcs;
  }
  return output;
}
// Function to find an object by key
function findObjectByKey(arr, keyToFind) {
...
function writePreloadTool() {
  var output = [];
  var preloadTool = getNextTool(tool.number != getFirstTool().number);
  if (getProperty("preloadTool") && preloadTool) {
    output.push("T" + toolFormat.format(preloadTool.number)); // preload next/first tool
  }
  return output;
}
// <<<<< INCLUDED FROM include_files/writeToolCall.cpi
  ...
  if (Array.isArray(coolantCodes)) {
    writeStartBlocks(!isOptionalCoolant, function () {
      if (settings.coolant.singleLineCoolant) {
        output.push(coolantCodes.join(getWordSeparator()));
      } else {
        for (var c in coolantCodes) {
          output.push(coolantCodes[c]);
        }
      }
    });
    return output;
...
function initializeSmoothing() {
  if (parseInt(getProperty("useSmoothing"), 10) == 9999) {
    settings.smoothing.roughing = getProperty("smoothingRoughing");
    settings.smoothing.semi = getProperty("smoothingSemiRoughing");
    settings.smoothing.semifinishing = getProperty("smoothingSemiFinishing");
    settings.smoothing.finishing = getProperty("smoothingFinishing");
  }
...
*/
function writeInitialPositioning(
  position,
  isRequired,
  codes1,
  codes2,
  ...
  writeStartBlocks(isRequired, function () {
    ...
    if (
      ...
      if (!insertToolCall && newWorkPlane) {
        onCommand(COMMAND_UNLOCK_MULTI_AXIS);
        writeln(formatComment(subst(localize("B AXIS ROTATION =  %1"), b)));
        writeBlock(
          gMotionModal.format(motionCode.multi),
          gAbsIncModal.format(90),
          wcsOutput,
        ...
        if (
          ...
          onDwell(getProperty("coolantDwell"));
        }
      } else {
        onCommand(COMMAND_UNLOCK_MULTI_AXIS);
        writeln(formatComment(subst(localize("B AXIS ROTATION =  %1"), b)));
...
var macroFormat = createFormat({
  prefix:
    typeof inspectionVariables == "undefined"
      ? "#"
...
function writeProbingToolpathInformation(cycleDepth) {
  ...
  if (isInspectionOperation()) {
    ...
    writeln(
      formatLocalVariable("DPRNT[CYCLEDEPTH*", 2, macroRoundingFormat + "]")
    );
  }
}
// <<<<< INCLUDED FROM include_files/commonInspectionFunctions_fanuc.cpi
